...
def main():
    ...
    path_to_image = i_arg.path_to_image
    checkpoint = i_arg.checkpoint
    top_k = i_arg.top_k
    cat_to_name = i_arg.cat_to_name
    gpu = i_arg.gpu
    ...
    prob_s, classes = predict(path_to_image, model, device, int(top_k))
   
    view_classify(prob_s, classes, cat_to_name)
...
def predict(image_path, model, device, topk=5):
    ...
    prob_s, classes = outputs[0].data.cpu().numpy()[0], outputs[1].data.cpu().numpy()[0]
    ...
    return prob_s, classes

def view_classify(prob_s, classes, cat_to_name):
